...
class Enemy(pygame.sprite.Sprite):  # fog class
    def __init__(self, pos_x, pos_y):
        super().__init__(player_group, all_sprites)
        self.image = load_image('fire.png')
        self.rect = self.image.get_rect().move(
    ...
    def find_path_step(self,start,target):
        x,y=start
        distance=[[INF]*30 for _ in range(6)]
        distance[y][x]=0
        prev = [[None] * 30 for _ in range(6)]
        queue=[(x,y)]
        while queue:
            x,y=queue.pop(0)
            for dx,dy in (1,0),(0,1),(-1,0),(0,-1):
                next_x,next_y=x+dx,y+dy
                if 0<=next_x<30 and 0<next_y<6 and \
                    distance[next_y][next_x]==INF:
                    distance[next_y][next_x]=distance[y][x]+50
        ...
        while prev[y][x]!=start:
            ...
            return x,y
    def move_enemy(self):
        next_position = self.find_path_step((self.pos_x,self.pos_y), (player.rect.x//50, player.rect.y//50))
        next_position=(next_position[0],next_position[1]+1)
        self.all_position += [next_position]
        print(self.all_position)
        self.rect = self.image.get_rect().move(
            tile_width * next_position[0], tile_height * next_position[1])
    def return_pos(self):
...
try:
    while running:  # game cycle
        ...
        enemy.move_enemy()
        fire=list(set(enemy.return_pos()))
        fire=[Fire(x) for x in fire]
